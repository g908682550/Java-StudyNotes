...
![1580971472541](Spring源码.assets/1580971472541.png)
			...
			2、如果没有返回bean，调用doCreateBean();
				...
				3、populateBean(beanName, mbd, instanceWrapper);
					...
					2、拿到InstantiationAwareBeanPostProcessor类型的后置处理器
						//这里会出现循环依赖问题，即单实例相互依赖，Spring默认支持循环依赖，在当前bean创建之前已经在第二个getSingleton方法中将该bean加入了spring的三级缓存，在该方法内会创建所依赖的bean，新bean依赖原bean时调用到这里时调用第一个getSingleton会发现上一个bean已经在创建过程中，因此不会再重复调用
...
![1580434579861](Spring源码.assets/1580434579861.png)
